...
namespace Lender.API.Application.Queries.Handlers
{
        ...
        IRequestHandler<NotBorrowedGamesQuery, GameDto[]>
    {
        ...
        {
            var notBorrowedGames = await _context.Games.Include(l => l.Loans).Where(x => !x.Loans.Any() || x.Loans.OrderByDescending(x => x.Id).FirstOrDefault().EndDate != null ).ToArrayAsync();
